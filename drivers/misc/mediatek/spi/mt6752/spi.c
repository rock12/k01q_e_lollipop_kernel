...
static int __init mt_spi_probe(struct platform_device *pdev)
{
	...
	if(pdev->dev.of_node)
	{
		...
		{
			if_config=0;
			dev_err(&pdev->dev, "SPI DTS doesn't defined pin, using DTC as default setting\n");
		}
		...
		{
			dev_err(&pdev->dev, "SPI DTS doesn't defined pin mode, using DTC as default setting\n");
		}
