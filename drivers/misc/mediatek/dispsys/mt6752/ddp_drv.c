...
void init_tplay_handle(void)
{
    void *va;
    va = dma_alloc_coherent(NULL, sizeof(unsigned int), &handle_pa, GFP_KERNEL);
    if (NULL == va)
    {
        DDPMSG("[SVP] failed to allocate handle_pa \n");
    }
...
static int __init disp_init(void)
{
    ...
    init_tplay_handle(); // non-zero value for valid VA
#endif
